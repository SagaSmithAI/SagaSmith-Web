...
| principal spoofing | principal derived from server session; never accepted from browser; MCP re-authorizes each call |
...
| CSRF | SameSite cookie; state-changing JSON endpoints; exact-origin proxy; Pack multipart still depends on SameSite and should add a double-submit token before cross-site embedding is allowed |
...
| malicious upload | hard byte limit, private bucket, non-executable object, checksum, rights attestation; MCP validates finalized archive before import |
| path traversal | server-generated object keys; resolved paths constrained to storage/exchange roots |
| SSRF | MCP and Agent URLs are deployment configuration, never user input; private network only |
...
monitoring network, add edge rate limits for auth/uploads/Agent calls, configure object encryption
